...
What *is* now shared: registration (`app/disha/registry.py`) and bucket ordering, capping and top-rank detection (`app/disha/core/curation.py`). Unifying the response contract is a deliberate non-goal for the moment — the current field names are what three shipped frontends and the UTMT portal integration already consume, so renaming them is a coordinated release, not a refactor. See the root README's "Adding a new exam" section.
