...
def test_meta_shape():
    ...
    assert {"OBC-NCL", "SC", "ST", "EWS"}.issubset(cat_values)
...
def test_recommend_endpoint_returns_results():
    ...
    # Subset rather than equality: `counts` has gained keys over time
    # (shown_by_category, is_curated, is_top_rank) and an exact-set assertion
    # turns every additive change into a failure. These four are the contract.
    assert {"total", "shown", "by_category", "by_type"} <= set(body["counts"])
